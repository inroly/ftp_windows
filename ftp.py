...
class MyFTP:
	...
	def connect(self):
		...
		self.ftp_port = int(self.ftp_port)
	...
	def download_file(self, local_path, remote_file_path):
		...
		with open(local_file_path, 'wb') as file_handle:
			self.ftp.retrbinary('RETR '+ remote_file_path, file_handle.write)
			print("flie download done")
 
if __name__ == '__main__':
	ftp = MyFTP('C:\\Users\\78252\\Desktop\\103\\ftp.conf')
	ftp.connect()
	ftp.login()
	# ftp.upload_folder('D:\\dir1')
	
	# time_start=time.time()
	# ftp.upload_file('D:\\img_align_celeba.zip','')
	# time_end=time.time()
	# print('totally upload cost',time_end-time_start)
	...
	ftp.download_file('D:\\', 'rand_file.bin')
	time_end_download=time.time()
	print('totally download cost',time_end_download-time_start_download)
